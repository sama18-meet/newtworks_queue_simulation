...
class Simulation:
    ...
    def __init__(self, total_time, num_servers, forwarding_prob_arr, _lambda, queue_max_size_arr, service_rate_arr):
        ...
        self.clock = 0.0
        self.next_arrival_time = generate_next_arrival();
    ...
    def advance_time(self):
        if (self.done):
            return
        next_event_time = min(self.next_arrival_time, min(server_next_done_time))
        ...
        if (next_event_time < self.total_time):
            if (self.next_arrival_time == next_event_time):
                self.next_arrival_time = handle_arrival_event()
        for i in range(num_servers):
            if (self.server_next_done_time[i] == next_event_time):
                ...
                if (self.server_curr_queue_size[i] > 0):
                    self.server_next_done_time[i] = generate_service_time()
                else:
                    self.server_next_done_time[i] = float('inf')
        if (self.clock >= self.total_time):
            if (server_curr_queue_size[i] == 0 for i in range(num_servers)):
    ...
    def handle_arrival_event(self):
        ...
        if (queue_max_size_arr[chosen_server] + 1 == server_curr_queue_size[chosen_server]):
            self.dropped += 1
        else:
            server_curr_queue_size[chosen_server] += 1
            self.num_in_system += 1
            if (server_curr_queue_size[chosen_server] == 1):
                self.server_next_done_time[chosen_server] = generate_service_time(chosen_server) 
            else:
                self.num_waiting += 1
        next_arrival_time = self.clock + self.generate_next_arrival()
...
if __name__ == '__main__':
    ...
    forwarding_prob_arr = []
    for i in range(num_servers):
        forwarding_prob_arr[i] = sys.argv[3+i]
    lambda_index = 3 + num_servers
    _lambda = sys.argv[lambda_index]
    queue_max_size_arr = []
    for i in range(num_servers):
        queue_max_size_arr[i] = sys.argv[lambda_index+i+1]
    service_rate_arr = []
    first_service_rate_index = lambda_index+num_servers+1
    for i in range(num_servers):
        service_rate_arr[i] = sys.argv[first_service_rate_index+i]
